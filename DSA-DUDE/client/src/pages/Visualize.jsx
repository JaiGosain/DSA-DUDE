    ...
    const validateCode = async () => {
        ...
        try {
            ...
            if (res.data.suggestedInput) {
                // Ensure suggested input is shown as pretty JSON string, not [object Object]
                setCustomInput(
                    typeof res.data.suggestedInput === "string"
                        ? res.data.suggestedInput
                        : JSON.stringify(res.data.suggestedInput, null, 2)
                );
            }
        } catch (error) {
    ...
    const visualize = async () => {
        ...
        try {
            ...
            try {
                ...
                // Just show a subtle visual cue; no long text block
                setInputError(true);
                                        ...
                                        <div className="mt-6 space-y-4">
                                            <div className="flex flex-col gap-2">
                                                ...
                                                <motion.input
                                                    ...
                                                    animate={inputError ? { x: [-4, 4, -3, 3, 0] } : { x: 0 }}
                                                    transition={{ duration: 0.3 }}
                                                    className={`w-full bg-black/40 border rounded-lg p-2 text-sm font-mono outline-none focus:border-orange-500/50 ${
                                                        inputError ? "border-red-500/70" : "border-white/10"
                                                    }`}
                                                />
                                            </div>
                                            <motion.button
